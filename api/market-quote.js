 ...
 export default async function handler(request) {
   ...
   if (!finnhubApiKey) {
     status = "fallback";
     fallbackReason = "Market data provider key is not configured. Showing demo data.";
   } else if (finnhubApiKey && quote) {
     try {
       ...
       if (finnhubResponse.ok) {
         ...
         if (finnhubData.c && finnhubData.c > 0) {
           ...
           status = "fallback";
           fallbackReason = "Market data provider request failed. Showing demo data.";
         }
       } else {
         status = "fallback";
         fallbackReason = "Market data provider request failed. Showing demo data.";
   ...
   // If symbol not found in demo (and no live data), return 404
   if (!quote) {
     return new Response(
       JSON.stringify({
